...
感谢你对 Crystal-Sculpt-Diffus-Coder 的兴趣！我们欢迎任何形式的贡献，包括新功能、工具插件、**技能**、文档改进、Bug 报告等。为了让协作更顺畅，请遵循以下流程和规范。

---

## 📦 工具贡献（最常用）

如果你想为 Crystal-Sculpt-Diffus-Coder 添加一个新工具（例如 `weather`、`calc` 等），请按以下步骤操作：

### 1. Fork 仓库
- 访问 [Crystal-Sculpt-Diffus-Coder 仓库](https://github.com/xhdlphzr/Crystal-Sculpt-Diffus-Coder)，点击右上角的 **Fork**，将仓库复制到你的 GitHub 账户下。
...
Crystal-Sculpt-Diffus-Coder 支持通过 `knowledge/` 文件夹加载额外的知识、规则或工作流（Markdown 文件）。你可以贡献一个实用的技能，帮助 AI 更好地完成特定领域的任务。
...
### 5. 本地验证
你可以在本地运行 Crystal-Sculpt-Diffus-Coder，观察技能是否被正常加载（启动时控制台会打印已加载的技能文件列表）。  
...
Crystal-Sculpt-Diffus-Coder 使用基于 YAML 的国际化方案。添加新语言步骤如下：
...
1. 将 Crystal-Sculpt-Diffus-Coder 官方仓库添加为远程：
   ```bash
...
## 🎉 感谢
感谢你愿意为 Crystal-Sculpt-Diffus-Coder 贡献！你的每一行代码和每一份知识都会让这个项目变得更好。如果遇到任何问题，欢迎在 PR 或 Issue 中提问。
