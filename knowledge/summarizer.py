...
MODULE_EXPERIENCE_PROMPT = """你是一个工程经验提炼师。分析以下单个模块的完整开发数据，提炼出该模块特有的工程经验。

模块数据包含以下层次：
- contract_data: 接口契约（签名、前置/后置条件、约束）
- logic_data: 算法步骤设计
- impl_data: 实现细节和代码结构
- incidents: L8 调试中遇到的故障和修复
- progress: 各阶段的完成时间戳
- events: 与该模块相关的开发事件

六维度参考价值：
- architecture: 模块设计、职责划分
- contract: 接口定义、数据契约
- algorithm: 算法流程、边界处理
- implementation: 代码组织、语言特性
- debug: 故障模式、根因分析
- meta: 开发节奏、进度管理

要求：
1. 每个经验必须基于提供的真实数据，不编造
2. 聚焦该模块的具体问题→解决方案→教训链条
3. 如果材料不足，返回空列表 []
...
CROSS_MODULE_SYNTHESIS_PROMPT = """你是一个高级工程架构师。对多个模块的开发经验进行跨模块综合，提炼出项目级的架构洞察和系统性教训。

以下数据供你分析：
1. 各模块提炼的经验（Stage 1 输出）：
{module_experiences}

2. 项目时间线：
{timeline}

3. 跨模块关注点（依赖图、架构、模块映射）：
{cross_cutting}
...
要求：
1. 识别重复出现的模式（如多个模块遇到同类问题）
2. 总结架构层面的决策教训（如模块划分的得失）
3. 提炼开发流程改进建议（如某阶段经常需要rollback的原因）
4. 输出比单模块经验更高层次的项目级经验
5. 只输出 JSON 数组

已有项目级经验（避免重复）：
{existing_titles}

返回格式：同单模块经验的 JSON 格式，但 module 字段设为 "__project__"，问题描述应有跨模块视角。
"""


PROJECT_REPORT_PROMPT = """你是一个技术文档撰写师。根据以下完整的项目开发数据，撰写一份综合性的《项目归档报告》。
...
## 3. 逐模块总结
- 对每个模块，按 contract → algorithm → implementation 链条总结
- 标注完成阶段和未完成阶段
- 列出该模块的故障数和关键修复

## 4. 开发时间线
- 关键事件（phase transition, rollback, module switch）
- 标注异常事件（回退、反复、长时间的阶段）

## 5. 故障与教训总结
- 按模块列出所有 TraceCrystal
- 识别共性问题

## 6. 经验结晶
- 列出生成的 ExperienceCrystal 标题和摘要
...
只输出 Markdown 正文，不要用代码块包裹。语言风格：专业、简洁、客观。
"""
